...
public interface ChiTietDonHangRepository extends JpaRepository<ChiTietDonHang, ChiTietDonHangId> {
       ...
       @Query("SELECT COALESCE(SUM(ct.soLuong), 0) FROM ChiTietDonHang ct WHERE ct.sach.maSach = :maSach AND ct.donHang.trangThai IN :trangThaiList")
       int sumSoLuongBySachAndDonHangTrangThaiIn(@Param("maSach") String maSach, @Param("trangThaiList") List<DonHang.TrangThaiDonHang> trangThaiList);
